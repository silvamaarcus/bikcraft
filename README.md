...
### 4. Executar o Servidor de Banco de Dados Mockado (Backend)
O projeto consome dados locais armazenados no arquivo `db.json`. Para rodar essa API mockada na porta `3001` (porta configurada no Axios), execute em um terminal separado:
```bash
npx json-server --watch db.json --port 3001
```

### 5. Executar o Servidor de Desenvolvimento (Frontend)
